<div class="bg-white shadow-lg rounded-2xl overflow-hidden ">
    <div class="p-8">
            ...
            <table class="w-full text-left">
                ...
                <tr class="bg-gray-50">
                    ...
                    <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider"
                        x-show="$wire.hasMissing">İşlem
                    </th>
                </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                @foreach($shipments as $index => $shipment)
                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out {{ $shipment['disabled'] ? 'opacity-50 bg-gray-100' : '' }}">
                        <td class="py-4 px-6" style="width: 40%">
                            <div>
                        ...
                        <td class="py-4 px-6" style="width: 37%">
                        ...
                        <td class="py-4 px-6 ">
                            <div class="flex space-x-4">
                                @if (!$shipment['disabled'])
                                    <button wire:click.prevent="removeShipment({{ $index }})"
                                            class="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                        <x-icon name="o-trash" class="w-5 h-5"/>
                                    </button>
                                @endif


                                <x-toggle label="Yazdırılabilir"
                                          wire:model.live.debounce="shipments.{{ $index }}.can_printable"
        ...
        <x-button wire:click.prevent="addShipment"
                  icon="o-plus"
                  label="Yeni Sevkiyat Ekle"
                  :disabled="!$hasMissing"
                  class="mt-8 px-6 py-3 bg-indigo-600 text-white hover:bg-indigo-700 transition duration-150 ease-in-out flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-lg">
        </x-button>
    </div>
</div>
