...
new class extends Component {
    ...
    public function delete(\App\Models\Unit $unit): void
    {
        if (\App\Models\Product::where('unit_id', $unit->id)->exists()) {
            throw \Mary\Exceptions\ToastException::error('Bu birim kullanımda, silinemez!');
        }

        $unit->delete();

        $this->success("{$unit->name} silindi");
    }
    ...
    public function units(): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
...
<div>
    <!-- HEADER -->
    <x-header title="Ölçü Birimleri" separator progress-indicator>
        ...
        <x-slot:actions>
            ...
            <x-button label="Yeni" link="/units/create" responsive icon="o-plus" class="btn-primary"/>
        </x-slot:actions>
    </x-header>

    <!-- TABLE  -->
    <x-card>
        <x-table link="units/{id}/edit" with-pagination :headers="$headers" :rows="$units" :sort-by="$sortBy">
            @scope('actions', $unit)
